...
// saves sorted data to "sorted.txt"
$contents = "";
    ...
    <tr> 資料匯出:
        ...
        <form action="main.php" style="display:inline" method="get">
            ...
            <select name="sort" onclick="this.form.submit()">
                <?php // print sorting options
                $column = array("", "出版社", "書名", "作者",
                                "定價", "發行日");
                foreach ($column as $key => $value) {
                    if ($key == 0)
                        continue;
                    if ($_GET['sort'] == $key)
                        echo "<option selected='selected' value='$key'>
                             $value</option>";
                    else
            ...
            方向:
            <select name="order" onclick="this.form.submit()">
